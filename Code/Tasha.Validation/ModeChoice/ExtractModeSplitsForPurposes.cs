...
{
    [ModuleInformation(Description = "This module is designed to be able to capture mode splits by zone, with the additional restriction that only included purposes will be accounted for.")]
    public class ExtractModeSplitsForPurposes : IPostHouseholdIteration
    {
        ...
        public void HouseholdIterationComplete(ITashaHousehold household, int hhldIteration, int totalHouseholdIterations)
        {
            foreach (var person in household.Persons)
            {
                ...
                foreach (var tripChain in person.TripChains)
                {
                    ...
                    {
                        if (Array.IndexOf(Activities, trip.Purpose) >= 0)
                        {
